...
export type EdgeTag = { key: string; short: string; blurb: string }
...
export function edgeTags(data?: MoneyEdgeData): EdgeTag[] {
  const tags: EdgeTag[] = []
  const add = (key: string, info?: { short: string; blurb: string }) => {
    if (info) tags.push({ key, short: info.short, blurb: info.blurb })
  }
